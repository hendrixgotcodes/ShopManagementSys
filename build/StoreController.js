...
function initialzeStoreItems() {
  _utilities_TableController__WEBPACK_IMPORTED_MODULE_3___default.a.showLoadingBanner("Please wait. Attempting to items in store...");
  database.fetchItems().then(fetchedItems => {
    ...
    if (fetchedItems.length > 0) {
      ...
      fetchedItems.forEach(fetchedItem => {
        _utilities_TableController__WEBPACK_IMPORTED_MODULE_3___default.a.createItem(fetchedItem.Name, fetchedItem.Brand, fetchedItem.Category, fetchedItem.Stock, fetchedItem.SellingPrice, "", false, fetchedItem.CostPrice, "", true, false, "Store");
      });
    } else {
      //Remove loading banner
...
function showItemsInCart() {
  console.log(cart);
  _modals_ModalController__WEBPACK_IMPORTED_MODULE_2__["default"].createCheckout(cart, totalSelectedRows, cartCount).then(totalCost => {
    if (totalCost >= 0) {
      salesMade = salesMade + totalCost; //Parsing it through a converter

      let forSpan = _utilities_UnitConverter__WEBPACK_IMPORTED_MODULE_4___default.a.convert(salesMade);
