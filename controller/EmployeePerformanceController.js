...
function initializeItems(){
    ...
    database.getTopEmployeesOrderedMonthly()
    .then((items)=>{
        ...
        items.forEach((item)=>{
            ...
            DOMCONTROLLER.createEmployeePerformanceItem(item.Name, item.SalesMade, item.ItemsSold, item.Revenue, item.Profit, item.Discount)
        })
...
function fetchAnalyticsData(e){
    ...
    if(toolBarDate_from.value === "" || toolBarDate_to.value === ""){
        ...
        database.getTopEmployees(toolBarDate_from.value, toolBarDate_to.value)
        .then((items)=>{
